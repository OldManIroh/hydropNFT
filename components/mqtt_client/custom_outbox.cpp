/**
 * @file custom_outbox.cpp
 * @brief Пример кастомного MQTT Outbox с использованием C++ memory resources
 * 
 * Этот файл демонстрирует создание пользовательского менеджера памяти
 * для MQTT outbox (очереди исходящих сообщений).
 * 
 * Назначение:
 * - Контроль использования памяти при публикации MQTT сообщений
 * - Логирование аллокаций и деаллокаций
 * - Использование std::pmr (polymorphic memory resources)
 * 
 * Как это работает:
 * 1. trace_resource - кастомный класс памяти, наследующий std::pmr::memory_resource
 * 2. Переопределяет do_allocate() и do_deallocate() для логирования
 * 3. Отслеживает общий объём выделенной памяти
 * 4. Используется с std::pmr::polymorphic_allocator для контейнеров
 * 
 * @note Этот файл НЕ используется в текущей конфигурации
 * @note Требуется COMPILER_CXX17 или новее для std::pmr
 * @note Для включения: раскомментировать в mqtt_client.c
 * 
